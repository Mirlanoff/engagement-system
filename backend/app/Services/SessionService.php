    ...
    public function processSnapshots(string $sessionId, array $snapshots): void
    {
        ...
        DB::transaction(function () use ($session, &$snapshots) {
            ...
            $records = array_map(fn($s) => [
                ...
                'posture_score'       => $s['posture_score'] ?? null,
                ...
                'hand_raised'         => $s['hand_raised'] ?? false,
                'attention_state'     => $s['attention_state'] ?? null,
                'confidence_overall'  => $s['confidence_overall'] ?? null,
                'not_detected_reason' => $s['not_detected_reason'] ?? null,
                'frame_quality'       => isset($s['frame_quality'])
                    ? json_encode($s['frame_quality']) : null,
                'score_breakdown'     => isset($s['score_breakdown'])
                    ? json_encode($s['score_breakdown']) : null,
