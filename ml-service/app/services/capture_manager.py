...
class CaptureSession:
    ...
    def _capture_loop(self, camera: dict, interval: int):
        ...
        try:
            ...
            logger.error("Invalid camera source", camera_id=camera_id, error=str(e))
            self._notify_laravel_camera_error(camera_id, str(e))
            return
        ...
        if not cap.isOpened():
            logger.error("Cannot open camera", camera_id=camera_id, source_type=type(source).__name__)
    ...
    def _resolve_camera_source(self, camera: dict) -> CameraSource:
        if camera.get("device_index") is not None:
            return int(camera["device_index"])

        source = camera.get("source") or camera.get("rtsp_url") or camera.get("url")
        if source is None:
        ...
        if source == "":
            raise ValueError("Camera source is empty")
        if source.startswith(("webcam://", "camera://")):
            index = source.split("://", 1)[1] or "0"
            return int(index)
        if source.startswith("/dev/video"):
            index = source.removeprefix("/dev/video")
            return int(index) if index.isdigit() else source
        if source.isdigit():
            return int(source)
